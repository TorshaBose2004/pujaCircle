...
export const PriestLoginPage: React.FC = () => {
  ...
  const onLogin = async (data: PriestLoginInput) => {
    ...
    if (success) {
      ...
      if (user?.role === "USER") {
        toast.info("Welcome Devotee! Redirecting to your sanctuary portal.");
        navigate("/user/home");
      } else if (user?.role === "ADMIN") {
        navigate("/admin/dashboard");
      } else {
        toast.success(
          "Pranam Acharya! Welcome to your ceremonial command altar.",
        );
        ...
        <div className="hidden lg:flex flex-col justify-between w-5/12 bg-[#780016] text-white p-8 sm:p-10 border-r-2 border-amber-400/40 relative">
          <div className="space-y-6">
            ...
            <div className="space-y-3 pt-2">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-sm bg-amber-400/20 text-amber-300 border border-amber-400/40 text-xs font-semibold">
                ...
                <span>Vedic Purohit Service</span>
              </div>
              <h2 className="text-2xl font-bold font-serif text-white leading-snug">
                Serve Devotees with Sacred Lineage
              </h2>
              <p className="text-xs text-amber-100/90 leading-relaxed">
                Accept verified puja ceremony appointments in your locality
                across West Bengal and receive 100% direct cash dakshina without
                commissions.
              </p>
            </div>
            ...
            <div className="space-y-2.5 pt-2">
              <div className="flex items-center gap-2 text-xs text-amber-100">
                ...
                <span>Zero commission — 100% direct cash dakshina</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-amber-100">
                <CheckCircle2 className="h-4 w-4 text-amber-400 shrink-0" />
                <span>Manage custom puja pricing & Muhurat slots</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-amber-100">
                <CheckCircle2 className="h-4 w-4 text-amber-400 shrink-0" />
                <span>Direct communication with verified devotees</span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-amber-400/30 space-y-1">
            <div className="text-xs font-serif text-amber-200 italic">
              “विद्वत्वं च नृपत्वं च नैव तुल्यं कदाचन — Sacred wisdom surpasses
              all royalty.”
            </div>
          ...
          <div>
            ...
            <div className="mb-6 pb-3 border-b border-stone-100">
              ...
              <p className="text-xs text-stone-500 mt-1">
                Access your ceremony requests and availability calendar
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit(onLogin)} className="space-y-4">
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <span>Enter Purohit Sanctum</span>
